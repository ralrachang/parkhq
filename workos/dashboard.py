...
def main():
    ...
    # 일별 SVG 막대
    dmax = max((d[2] for d in daily), default=1) or 1
    bw = 26; gap = 8; H = 150
    svg = []
    for i, (day, sess, tok) in enumerate(daily):
        h = (H-20) * tok / dmax
        x = i*(bw+gap)
...
<title>PARK HQ Work OS — 대시보드 미리보기</title>
<style>
:root{{--bg:#0d1117;--card:#161b22;--bd:#30363d;--fg:#e6edf3;--mut:#8b949e;--acc:#3b82f6;--acc2:#22c55e;}}
*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--fg);
font-family:'Pretendard',-apple-system,'Segoe UI',Roboto,'Malgun Gothic',sans-serif;font-size:14px}}
.wrap{{max-width:1180px;margin:0 auto;padding:28px 22px 60px}}
h1{{font-size:22px;margin:0 0 4px}}h2{{font-size:15px;margin:30px 0 12px;color:var(--fg)}}
.sub{{color:var(--mut);font-size:13px}}
.badge{{display:inline-block;background:#3b1d1d;color:#ff7b72;border:1px solid #5c2a2a;
border-radius:6px;padding:3px 9px;font-size:12px;font-weight:600;margin-left:8px}}
...
.bar{{display:grid;grid-template-columns:150px 1fr 92px;align-items:center;gap:10px;margin:7px 0}}
.bl{{color:var(--fg);font-size:12.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}
.bt{{background:#0d1117;border-radius:5px;height:16px;overflow:hidden}}
.bf{{background:linear-gradient(90deg,var(--acc),#60a5fa);height:100%}}
...
th{{color:var(--mut);font-weight:600}}td.r,th.r{{text-align:right;font-variant-numeric:tabular-nums}}
svg text.dx{{fill:var(--mut);font-size:9px;text-anchor:middle}}
rect.db{{fill:var(--acc2)}}
.note{{color:var(--mut);font-size:12px;margin-top:8px;line-height:1.5}}
.ok{{color:var(--acc2);font-weight:600}}
.foot{{margin-top:34px;color:var(--mut);font-size:11.5px;border-top:1px solid var(--bd);padding-top:14px;line-height:1.7}}
</style></head><body><div class="wrap">
<h1>PARK HQ Work OS — 업무 대시보드 <span class="badge">🔒 로컬 전용 · 비공개</span></h1>
<div class="sub">Phase 2 미리보기 · 데이터 {drange[0]} ~ {drange[1]} (KST) · Claude Code 세션 로그 {files_seen}파일 정규화 적재 ·
적재 무결성 <span class="ok">게이트 12/12 PASS</span></div>

<div class="kpis">
<div class="kpi"><div class="v">{n_ws_active}<span style="font-size:13px;color:var(--mut)"> / {n_ws}</span></div><div class="l">활동 워크스페이스</div></div>
<div class="kpi"><div class="v">{n_sess:,}</div><div class="l">세션</div></div>
<div class="kpi"><div class="v">{n_human:,}</div><div class="l">사람 입력(typed)</div></div>
<div class="kpi"><div class="v">{tout/1e6:.1f}M</div><div class="l">출력 토큰(dedup 진실)</div></div>
<div class="kpi"><div class="v">${cost_total:,.0f}</div><div class="l">추정 비용(가정 단가)</div></div>
<div class="kpi"><div class="v">{stored:,}</div><div class="l">정규화 레코드</div></div>
</div>

<h2>일별 작업량 추이 — 출력 토큰 (최근 {len(daily)}일, KST)</h2>
<div class="card"><svg width="{svgw}" height="{H}" viewBox="0 0 {svgw} {H}">{''.join(svg)}</svg>
<div class="note">막대 = 그날 생성된 출력 토큰량(작업 산출 강도의 대리지표).</div></div>
...
<div class="card"><h2 style="margin-top:0">비용 추정 <span class="sub">· 가정 단가, 확인 필요</span></h2>
...
토큰 정직성: 순진 합산 {raw_out/1e6:.1f}M 중 resume 재기록 {(raw_out-canon_out)/1e6:.1f}M(7.4%)을 dedup 제외 → 진실값 {canon_out/1e6:.1f}M 사용.<br>
멱등키=content_hash(봉투필드 제외) · 무결성 게이트 12/12 PASS · <b>이 파일은 로컬 전용</b>(고객명·세션제목 포함 가능, 커밋 금지).
</div>
</div></body></html>"""
